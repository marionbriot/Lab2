...
//	MENU_TRIER_VALEURS_PAR_ORDRE_ORIGINAL = "app.frame.menus.sortBy.OriginalOrder";
	...
	public MenuFenetre(CommBase comm) {
		...
		addMenuFichier();
		addMenuAide();
		System.out.println(sortBySequenceCroissante.toString());
	}
	/**
	 *  Créer le menu "Draw". 
	 */
	...
	private void addMenuTrierOptions() {
		sortBySequenceCroissante = new JRadioButtonMenuItem("MENU_TRIER_TITRE");
		System.out.println(sortBySequenceCroissante.toString());
		JMenu menu = creerMenu (MENU_TRIER_TITRE, new JRadioButtonMenuItem[] { sortBySequenceCroissante });
		menu.getItem(0).addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
...
//		});
	...
	private static JMenu creerMenu(String titleKey,JRadioButtonMenuItem[] itemKeys) {
		JMenu menu = new JMenu(LangueConfig.getResource(titleKey));
		for(int i=0; i < itemKeys.length; ++i) {
			String itemKeysToString = "app.frame.menus.sortBy.title";
			menu.add(new JMenuItem(LangueConfig.getResource(itemKeysToString)));
		}
		return menu;
	}
}